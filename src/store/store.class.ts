...
export class Store {
    private store: BehaviorSubject<ServerStatus[]> = new BehaviorSubject([]);
    ...
    updateServerStatus(status: ServerStatus): void {
        ...
        const index = latestValue.findIndex((item) => item.server === status.server);
    ...
    private hasServerStatusChanged(current: ServerStatus, previous: ServerStatus): boolean {
        if (current.online !== previous.online) {
            return true;
        }

        if ((!!current.players && !previous.players) || (!current.players && !!previous.players)) {
            return true;
        }
        ...
        if (!!current.players && !!previous.players) {
            ...
            return diff(currentPlayers, previousPlayers).length > 0;
        }
    }
